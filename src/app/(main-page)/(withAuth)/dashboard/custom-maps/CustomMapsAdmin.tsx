...
const MappoolMapsSection = ({ maps, onToggle, modColors }: MappoolMapsSectionProps) => {
	...
	return (
		<div className="flex flex-col gap-3">
			<div>
				<h2 className="text-lg font-semibold">Mappool custom maps</h2>
			...
			{Object.entries(byTournament).map(([tournamentName, stages]) => {
				...
				return (
					<div key={tournamentName} className="rounded-lg bg-gray-800/50 overflow-hidden">
						<button
							type="button"
							onClick={() => toggleTournament(tournamentName)}
							className="flex w-full items-center justify-between px-4 py-3 text-left hover:bg-gray-700/50 transition-colors"
						>
							<div className="flex items-center gap-3">
								<span className="font-semibold">{tournamentName}</span>
								<span className="text-xs text-gray-400">{totalMaps} maps</span>
								{totalOriginals > 0 && (
							...
							<span className="text-gray-400 text-sm">{isTOpen ? "▲" : "▼"}</span>
						</button>
						...
						{isTOpen && (
							<div className="border-t border-gray-700 px-4 py-3 flex flex-col gap-2">
								{Object.entries(stages).map(([stageName, stageMaps]) => {
									...
									return (
										<div key={stageKey} className="rounded-lg bg-gray-900/60 overflow-hidden">
											<button
												type="button"
												onClick={() => toggleStage(stageKey)}
												className="flex w-full items-center justify-between px-3 py-2 text-left hover:bg-gray-700/40 transition-colors"
											>
												<div className="flex items-center gap-3">
													<span className="text-sm font-medium text-gray-200">
														{STAGE_LABELS[stageName] ?? stageName}
													</span>
													<span className="text-xs text-gray-500">{stageMaps.length} maps</span>
													{stageOriginals > 0 && (
												...
												<span className="text-gray-500 text-xs">{isSOpen ? "▲" : "▼"}</span>
											</button>

											{isSOpen && (
												<div className="border-t border-gray-800 flex flex-col gap-1.5 p-2">
													{stageMaps.map((map) => (
														<div
															key={map.id}
														...
														>
															<div className="flex items-center gap-3 min-w-0">
																<span
																	className={`shrink-0 rounded px-2 py-0.5 text-xs font-bold text-white ${modColors[map.modification ?? ""] ?? "bg-gray-600"}`}
																>
																	{map.modification}
																</span>
																<div className="min-w-0">
																	<p className="truncate text-sm font-medium">{map.title}</p>
																	<p className="truncate text-xs text-gray-400">{map.artist}</p>
																</div>
															</div>
															<button
																onClick={() => onToggle(map.id, map.isCustomSong ?? false)}
																className={`ml-3 shrink-0 rounded px-3 py-1 text-xs font-semibold transition-colors ${
																	map.isCustomSong
...
export const CustomMapsAdmin = ({ maps: initialMaps, mappoolMaps: initialMappoolMaps }: CustomMapsAdminProps) => {
	...
	const handleToggleMappoolCustomSong = async (beatmapId: string, current: boolean) => {
		const result = await toggleMappoolBeatmapCustomSongAction(beatmapId, !current);
		if (!result.status) {
			toast.error(result.errorMessage);
			return;
		}
		setMappoolMaps((prev) => prev.map((m) => (m.id === beatmapId ? result.data : m)));
	};
	...
	const handleRefreshPlayCounts = async () => {
		...
		if (!result.status) {
			...
			toast.success("Play counts refreshed.");
		}
	};
		...
		<div className="flex flex-col gap-8">
			...
			<div className="flex flex-col gap-3">
				<div className="flex items-center justify-between">
					...
					>
						{refreshing ? "Refreshing..." : "Refresh play counts"}
					</Button>
				</div>
				...
				{Object.entries(byEdition).map(([edition, editionMaps]) => {
					...
					return (
						<div key={edition} className="rounded-lg bg-gray-800/50 overflow-hidden">
							{/* Edition header */}
							<button
								type="button"
							...
							{/* Map rows */}
							{isOpen && (
								<div className="border-t border-gray-700 flex flex-col gap-1.5 p-2">
									{editionMaps.map((map) => (
										<div key={map.id} className="flex flex-col rounded-lg bg-gray-900/60 overflow-hidden">
											...
											{editingId === map.id && editState && (
												<div className="border-t border-gray-700 bg-gray-900/80 px-4 py-4 flex flex-col gap-4">
													...
													<label className="flex cursor-pointer items-center gap-3">
														...
														<span className="text-sm font-medium">
															Original song{" "}
															<span className="text-gray-400 font-normal">(music was made specifically for this tournament)</span>
														</span>
													</label>
													<Button
														type="button"
			...
			<MappoolMapsSection
				maps={mappoolMaps}
				onToggle={handleToggleMappoolCustomSong}
				modColors={MOD_COLORS}
			/>
		</div>
	);
};
