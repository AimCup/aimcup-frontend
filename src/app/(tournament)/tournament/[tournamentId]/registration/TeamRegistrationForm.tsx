...
const TeamRegistrationForm = ({ tournamentId }: { tournamentId: string }) => {
    ...
    const [_state, formAction] = useTypeSafeFormState(createTeamSchema, async (data) => {
        ...
        if (!teamResponseDto.status) {
            return toast.error(teamResponseDto.errorMessage, { duration: 3000 });
        }
        if ("response" in teamResponseDto) {
            formRef.current?.reset();
            router.push(`/tournament/${tournamentId}/teams/${teamResponseDto.response.id}`);
        }
    });
